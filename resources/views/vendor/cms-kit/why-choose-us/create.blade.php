...
<div class="card">
    ...
    <div class="card-body p-4">
        ...
        @endif
        <form action="{{ route('cms.why-choose-us.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
                ...
                <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="{{ $lang->code }}-content" role="tabpanel">
                    <div class="row g-4">
                        <div class="col-12">
                            ...
                            <input type="text" name="translations[{{ $lang->code }}][title]" class="form-control @error("translations.{$lang->code}.title") is-invalid @enderror" value="{{ old("translations.{$lang->code}.title") }}" placeholder="e.g. Select Property" required>
                            @error("translations.{$lang->code}.title")<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-bold">Description</label>
                            <textarea name="translations[{{ $lang->code }}][description]" class="form-control" rows="3">{{ old("translations.{$lang->code}.description") }}</textarea>
                        </div>
                    </div>
                </div>
                @endforeach
            ...
            <div class="mt-5 d-flex gap-2">
                ...
                <a href="{{ route('cms.why-choose-us.index') }}" class="btn btn-outline-secondary px-4">Cancel</a>
            </div>
        </form>
    </div>
</div>
