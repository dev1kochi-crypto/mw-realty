...
<style>
    ...
    .hero-chip-label { display: block; font-size: 0.65rem; opacity: 0.75; font-weight: 700; text-transform: uppercase; letter-spacing: 0.03em; }
    ...
    .stat-mini:hover { transform: translateY(-3px); box-shadow: 0 10px 24px rgba(28,35,64,0.1); color: inherit; }
    ...
    .dash-card { border-radius: 16px; border: 1px solid var(--dash-border); background: #fff; box-shadow: 0 4px 14px rgba(28,35,64,0.05); padding: 1.25rem 1.4rem; height: 100%; }
    ...
    .pipeline-card.tone-teal  { background: rgba(4,161,204,0.08); }
    .pipeline-card.tone-amber { background: rgba(224,142,11,0.08); }
    .pipeline-card.tone-green { background: rgba(15,157,88,0.08); }
    ...
    .dash-tabs .nav-link.active { background: var(--dash-navy); color: #fff; }
    ...
    .btn-period.active { background: var(--dash-teal); border-color: var(--dash-teal); color: #fff; }
    .btn-period:hover { color: var(--dash-navy); }
...
<div class="row g-2 mb-3">
    <div class="col-6 col-md-3 col-xl">
        <a href="{{ route('portal.properties.index') }}" class="stat-mini">
    ...
    <div class="col-6 col-md-3 col-xl">
        <a href="{{ route('cms.portal-accounts.index') }}" class="stat-mini">
    ...
    <div class="col-6 col-md-3 col-xl">
        <a href="{{ route('portal.crm.index') }}" class="stat-mini">
    ...
    <div class="col-6 col-md-3 col-xl">
        <a href="{{ route('portal.crm.index', ['status' => 'new']) }}" class="stat-mini">
    ...
    <div class="col-6 col-md-3 col-xl">
        <a href="{{ route('cms.plans.index') }}" class="stat-mini">
...
<div class="dash-card mb-3">
    ...
    <div class="pipeline-v2">
        <div class="pipeline-card tone-teal">
            <div class="pc-top">
                <span class="pc-icon fill-teal"><i class="fas fa-inbox"></i></span>
                <span class="pc-pct">{{ round($newCount / $pipelineTotal * 100) }}%</span>
            </div>
            <div class="pc-num">{{ $newCount }}</div>
            <div class="pc-label">New</div>
            <div class="pc-track"><div class="pc-fill fill-teal" style="width: {{ round($newCount / $pipelineTotal * 100) }}%;"></div></div>
        </div>
        <div class="pipeline-arrow"><i class="fas fa-chevron-right"></i></div>
        <div class="pipeline-card tone-amber">
            <div class="pc-top">
                <span class="pc-icon fill-amber"><i class="fas fa-comments"></i></span>
            ...
            <div class="pc-track"><div class="pc-fill fill-amber" style="width: {{ round($contactedCount / $pipelineTotal * 100) }}%;"></div></div>
        </div>
        <div class="pipeline-arrow"><i class="fas fa-chevron-right"></i></div>
        <div class="pipeline-card tone-green">
            <div class="pc-top">
                <span class="pc-icon fill-green"><i class="fas fa-check-double"></i></span>
                <span class="pc-pct">{{ round($closedCount / $pipelineTotal * 100) }}%</span>
            </div>
            <div class="pc-num">{{ $closedCount }}</div>
            <div class="pc-label">Closed</div>
            <div class="pc-track"><div class="pc-fill fill-green" style="width: {{ round($closedCount / $pipelineTotal * 100) }}%;"></div></div>
        </div>
    </div>
</div>
                ...
                <div class="tab-pane fade show active" id="tab-owners">
                    <div class="performer-list">
                        ...
                        <div class="performer-row">
                            <div class="performer-avatar {{ $owner->type === 'company' ? 'fill-navy' : 'fill-teal' }}">{{ strtoupper($initials) ?: '?' }}</div>
                            ...
                            <div class="performer-stats">
                                ...
                                <span class="performer-stat"><i class="fas fa-address-book"></i> {{ $owner->enquiries_count }}</span>
                            </div>
                        </div>
                        @empty
    ...
    <div class="col-lg-4 d-flex flex-column gap-3">
        <div class="dash-card">
            ...
            <div class="action-row">
                ...
                <a href="{{ route('portal.crm.index', ['status' => 'new']) }}" class="btn btn-sm btn-outline-secondary py-0">View</a>
            </div>
            @endif
        ...
        <div class="dash-card">
            ...
            <div class="breakdown-bar-track" style="height: 12px;">
                <div class="breakdown-bar-fill fill-teal" style="width: {{ $stats['approval_rate'] }}%;"></div>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="dash-card">
            ...
            @forelse($planDistribution as $plan)
                <div class="breakdown-row">
                    ...
                    <div class="breakdown-bar-track"><div class="breakdown-bar-fill fill-navy" style="width: {{ round($plan->subscribers_count / $planTotal * 100) }}%;"></div></div>
    ...
    <div class="col-lg-5">
        <div class="dash-card h-100">
            ...
            <div class="action-btn-stack d-grid gap-2">
                <a href="{{ route('portal.properties.create') }}" class="btn fill-teal"><i class="fas fa-plus-circle"></i> Add New Property</a>
                @if(config('cms-kit.common.modules.banners', true) && $cmsUser->can('banners.edit'))
                <a href="{{ route('cms.banners.create') }}" class="btn fill-navy"><i class="fas fa-image"></i> Add New Banner</a>
                @endif
                @if($cmsUser->can('portal-accounts.view'))
                <a href="{{ route('cms.portal-accounts.index') }}" class="btn fill-amber"><i class="fas fa-user-check"></i> Review Agents & Companies</a>
                @endif
                @if($cmsUser->can('plans.view'))
                <a href="{{ route('cms.plans.index') }}" class="btn fill-green"><i class="fas fa-layer-group"></i> Manage Plans</a>
                @endif
                @if($cmsUser->can('filters.view'))
                <a href="{{ route('cms.filters.index') }}" class="btn fill-navy"><i class="fas fa-sliders-h"></i> Manage Filters</a>
                @endif
