    ...
    <div class="col-12">
        <div class="card mb-4 border-0 shadow-sm">
            <div class="card-header bg-white py-3">
                <h6 class="mb-0 fw-bold text-primary">"Popular Properties in Communities" Section Settings</h6>
            </div>
            <div class="card-body p-4">
                <div class="alert alert-light border-start border-primary border-4 py-2 mb-4 shadow-sm" style="font-size: 0.9rem;">
                    <i class="fas fa-info-circle text-primary me-2"></i>
                    Each community card below links to a value of the <strong>Location</strong> filter, so make sure the
                    communities you want here already exist as options under <a href="{{ route('cms.filters.index') }}">Filters &raquo; Location</a>.
                </div>
                <form action="{{ route('cms.communities.update-section') }}" method="POST">
                    @csrf
                        ...
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="section-panel-{{ $lang->code }}" role="tabpanel">
                            <div class="row g-4">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="translations[{{ $lang->code }}][title_1]" class="form-control @error("translations.{$lang->code}.title_1") is-invalid @enderror" value="{{ old("translations.{$lang->code}.title_1", $section->translations[$lang->code]['title_1'] ?? '') }}" placeholder="e.g. Neighbourhood living">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="translations[{{ $lang->code }}][title]" class="form-control @error("translations.{$lang->code}.title") is-invalid @enderror" value="{{ old("translations.{$lang->code}.title", $section->translations[$lang->code]['title'] ?? '') }}" placeholder="Popular Properties in Dubai Communities" required>
                    ...
                    <div class="row g-4">
                        <div class="col-md-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="listing_status_filter_enabled" id="listingStatusFilter" {{ old('listing_status_filter_enabled', $extra['listing_status_filter_enabled'] ?? true) ? 'checked' : '' }}>
                                <label class="form-check-label fw-bold" for="listingStatusFilter">Listing Status Filter </label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" name="display_home" id="displayHome" {{ old('display_home', $extra['display_home'] ?? true) ? 'checked' : '' }}>
                                <label class="form-check-label fw-bold" for="displayHome">Display on Home Page</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-check form-switch">
        ...
        <div class="card">
            <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
                ...
                <div class="d-flex gap-2">
                    @if($cmsUser->can('communities.delete'))
                    ...
                    @endif
                    @if($cmsUser->can('communities.create'))
                    <a href="{{ route('cms.communities.create') }}" class="btn btn-primary btn-sm">
...
<script>
    $(function() {
        const table = $('.premium-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('cms.communities.index') }}",
            columns: [
        ...
        $(document).on('change', '.toggle-status', function() {
            ...
            const url = "{{ url(config('cms-kit.common.auth.prefix', 'admin')) }}/communities/" + id + "/toggle-status";
        ...
        $(document).on('change', '.reorder-input', function() {
            ...
            $.post("{{ route('cms.communities.reorder') }}", { _token: '{{ csrf_token() }}', id: id, order_index: order })
        ...
        $(document).on('click', '.delete-item', function() {
            if (confirm('Are you sure you want to delete this community?')) {
                const id = $(this).data('id');
                const url = "{{ url(config('cms-kit.common.auth.prefix', 'admin')) }}/communities/" + id;
        ...
        window.bulkAction = function(action) {
            ...
            $.post("{{ route('cms.communities.bulk-action') }}", { _token: '{{ csrf_token() }}', action: action, ids: ids })
                .done(function() {
