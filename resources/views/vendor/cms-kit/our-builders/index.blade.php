...
@section('breadcrumbs')
    <li class="breadcrumb-item active" aria-current="page">Our Builders</li>
@endsection
    ...
    <div class="col-12">
        <div class="card mb-4 border-0 shadow-sm">
            <div class="card-header bg-white py-3">
                <h6 class="mb-0 fw-bold text-primary">"Our Builders" Section Settings</h6>
            </div>
            <div class="card-body p-4">
                <form action="{{ route('cms.our-builders.update-section') }}" method="POST">
                    @csrf
                    ...
                    <div class="tab-content mb-4 language-switcher-content">
                        ...
                        <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="section-panel-{{ $lang->code }}" role="tabpanel">
                            ...
                            <input type="text" name="translations[{{ $lang->code }}][title]" class="form-control @error("translations.{$lang->code}.title") is-invalid @enderror" value="{{ old("translations.{$lang->code}.title", $section->translations[$lang->code]['title'] ?? '') }}" placeholder="Our Builders" required>
        ...
        <div class="card">
            <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
                <h5 class="mb-0">Builder Logos</h5>
                <div class="d-flex gap-2">
                    @if($cmsUser->can('our-builders.delete'))
                    ...
                    @endif
                    @if($cmsUser->can('our-builders.create'))
                    <a href="{{ route('cms.our-builders.create') }}" class="btn btn-primary btn-sm">
                        <i class="fas fa-plus"></i> Add Builder
                    </a>
                    @endif
                </div>
            </div>
...
<script>
    $(function() {
        const table = $('.premium-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('cms.our-builders.index') }}",
            columns: [
        ...
        $(document).on('change', '.toggle-status', function() {
            ...
            const url = "{{ url(config('cms-kit.common.auth.prefix', 'admin')) }}/our-builders/" + id + "/toggle-status";
        ...
        $(document).on('change', '.reorder-input', function() {
            ...
            $.post("{{ route('cms.our-builders.reorder') }}", { _token: '{{ csrf_token() }}', id: id, order_index: order })
        ...
        $(document).on('click', '.delete-item', function() {
            if (confirm('Are you sure you want to delete this builder?')) {
                const id = $(this).data('id');
                const url = "{{ url(config('cms-kit.common.auth.prefix', 'admin')) }}/our-builders/" + id;
        ...
        window.bulkAction = function(action) {
            ...
            $.post("{{ route('cms.our-builders.bulk-action') }}", { _token: '{{ csrf_token() }}', action: action, ids: ids })
                .done(function() {
