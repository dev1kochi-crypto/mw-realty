...
<div class="card">
    ...
    <div class="card-body p-4">
        ...
        @endif
        <form action="{{ route('cms.popular-places.update', $place->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="tab-content mb-4 language-switcher-content">
                @foreach($languages as $lang)
                @php $t = $place->translations[$lang->code] ?? []; @endphp
            ...
            <div class="row g-4">
                <div class="col-md-6">
                    ...
                    @error('image')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <input type="text" name="image_alt" class="form-control mt-2" placeholder="Image ALT text" value="{{ old('image_alt', $place->image_alt) }}">
                    @if($place->image)
                        <div class="mt-2">
                            <img src="{{ asset('storage/' . $place->image) }}" class="img-thumbnail" style="height: 60px;">
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="order_index" class="form-control" value="{{ old('order_index', $place->order_index) }}" min="1">
                </div>
                <div class="col-md-3 d-flex align-items-end pb-2">
                    <div class="form-check form-switch mb-0">
                        <input class="form-check-input" type="checkbox" name="status" id="placeStatus" {{ old('status', $place->status) ? 'checked' : '' }}>
...
<script>
    ...
    // a display:none field), which looks exactly like "Update does nothing". Catch that native
